...
unsigned long GetUserDirectoryTableBaseOffset() //located in KPROCESS->UserDirectoryTableBase
{
	...
	case WINDOWS_21H1:
		...
		break;
	default:
		return 0x0388; //default is everything after because no change till win 11 22H2
	}
}

//check normal dirbase if 0 then get from UserDirectoryTableBase
...
NTSTATUS ReadProcessMemory(int pid, PVOID Address, PVOID AllocatedBuffer, SIZE_T size, SIZE_T* read)
{
	...
	if (!process_dirbase) return STATUS_UNSUCCESSFUL;
